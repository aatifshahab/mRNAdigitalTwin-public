...
%phi_elec2 = 4*pi*e_0*e_rel*Length(i)*Length(j)/( Length(i) + Length(j) )*potential^2*log(1+exp(-kappa.*a));



phi_total = phi_VDW + phi_elec;

%W1 = sum(  exp(phi_elec2/kB/T).*( Length(i) + Length(j) + a ).^(-2)*delta2 )*(Length(i) + Length(j));
%W2 = sum(  exp(phi_VDW/kB/T).*( Length(i) + Length(j) + a ).^(-2)*delta2 )*(Length(i) + Length(j));
...
%%
% plot(a*1000000000, phi_VDW)
% hold on
% plot(a*1000000000, phi_elec)
% hold on
% plot(a*1000000000, exp(phi_total/kB/T).*( Length(i) + Length(j) + a ).^(-2))
% legend('VDW', 'Electrostatic', 'Total')
% xlim([0 300])
% 
