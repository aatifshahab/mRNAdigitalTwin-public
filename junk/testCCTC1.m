function [tSol, unbound_mRNA, bound_mRNA] = testCCTC1(states0_last_value)
    ...
    % --------------------------- Solve the ODE ------------------------------
    [tSol_seconds, states_matrix] = ode15s(@(t, st) CCTC_to_aatif(st, params), t_vec, states0);

    % stop the simulation on reaching saturation
    % opts = odeset('Events', @(t,y) satEvent(t,y,params));
    % [tSol_s, states, te, ye, ie] = ode15s(@(t,y) CCTC_to_aatif(y,params), t_vec, states0, opts);
    % tSol = tSol_s / 3600;
...
function [value,isterminal,direction] = satEvent(t, y, params)
    % y contains [c; q; cs].  We only care about q here.
    n = params.n; nbin = params.nbin;
    q = reshape(y(n*nbin+1:2*n*nbin), [n, nbin]);
    % compute current avg bound:
    V = params.V; epsilonp = params.epsilonp;
    resinVol = (1-epsilonp)*V;
    avgBound = sum(q(:).*resinVol(:)) / sum(resinVol(:));
    % stop when avgBound is within 0.01 of qmax
    value      = avgBound - (params.qmax - 1e-2);
    isterminal = 1;   % stop the integration
    direction  = +1;  % only trigger when value increases through zero
end